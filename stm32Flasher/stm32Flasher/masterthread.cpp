...
QByteArray MasterThread::create_request(int cmd, QByteArray* arr)
{
    ...
    if (cmd == BTL_CMD_SETDATA_FLASH)
    {
        req[4] = arr->size() >> 8;
        req[5] = arr->size();
    }
    else if (cmd == BTL_CMD_FLASH)
    {
        req.append(*arr);
    }
        ...
        else
        {
            ...
            else
            {
                ...
                while (!currentRequest.isEmpty())
                {
                    ...
                    QByteArray cmd_setdataflash = create_request(BTL_CMD_SETDATA_FLASH, &requestData);//
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        {
                            case 0:
                                if (send_request(cmd_setdataflash, BTL_CMD_SETDATA_FLASH) == RESPONSE_ERROR)
                                    needClean = true;
                                //send_request(cmd_setdataflash, BTL_CMD_SETDATA_FLASH);
                                break;

                            case 1:
                                if (send_request(cmd_flash, BTL_CMD_FLASH) == RESPONSE_ERROR)
                                    needClean = true;
                                //send_request(cmd_flash, BTL_CMD_FLASH);
                                break;
